...
plot_data$Deviation <- plot_data$Secondary_CPUE - plot_data$Primary_CPUE

plot_data$`Proportional Deviation` <- (plot_data$Secondary_CPUE - plot_data$Primary_CPUE)/plot_data$Primary_CPUE

plot_data$`Proportional Deviation` <- sapply(plot_data$`Proportional Deviation`, function(x) ifelse(x == Inf, 1, x))

plot_data$Species <- factor(plot_data$Common_Name, levels = c("Octopus", "Thornyhead", "Sixgill Shark", "Darkblotched Rkfish"))

melt_plot_data <- melt(plot_data, id.vars = c("Species", "Year"), measure.vars = c("Deviation", "Proportional Deviation"), variable_name = "Deviations")

plot_image <- ggplot(melt_plot_data, aes(Species, value, col = factor(Year), shape = Species)) + geom_point(position = "jitter") + geom_abline(slope = 0, intercept = 0) + labs(x = "Species", y = "Deviation", col = "Year") + theme_bw() + facet_wrap(~ Deviations, scales = "free_y", nrow = 2)
...
plot_data2$Deviation <- plot_data2$secCPUE - plot_data2$priCPUE

plot_data2$`Proportional Deviation` <- (plot_data2$secCPUE - plot_data2$priCPUE)/plot_data2$priCPUE

plot_data2$`Proportional Deviation` <- sapply(plot_data2$`Proportional Deviation`, function(x) ifelse(x == Inf, 1, x))

plot_data2$Species <- factor(plot_data2$Common_Name, levels = c("Octopus", "Shortspine thornyhead", "Lingcod", "Pacific cod"))

melt_plot_data2 <- melt(plot_data2, id.vars = "Species", measure.vars = c("Deviation", "Proportional Deviation"), variable_name = "Deviations")

plot_image2 <- ggplot(melt_plot_data2, aes(Species, value, shape = Species)) + geom_point(position = "jitter") + geom_abline(slope = 0, intercept = 0) + labs(x = "Species", y = "Deviation") + theme_bw() + facet_wrap( ~ Deviations, scales = "free_y", nrow = 2)
