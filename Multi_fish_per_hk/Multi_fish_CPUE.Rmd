...
=======
#########################
#here's s nice trick for install/loading required libraries, so you don't have to source to another file
...
>>>>>>> 39e7b2489c8342788efbec413e1a18b46f81a409
...
=======
Sample <- matrix(ncol = Y, nrow = I) #NOTE: "sample" is a base function, here you capitalized it so it doesn't overwrite the function,
#but in practice, probably better to use a more descriptive name, e.g., "Sample_XXX" Not worth changing, for future reference.
>>>>>>> 39e7b2489c8342788efbec413e1a18b46f81a409
...
=======
  row <- match(spec, unique(unq_key$Common_Name)) #same with my comment on "Sample", suggest using something like "row_i"
  col <- match(year, unique(unq_key$Year)) #"col_i", not worth changing at this point, but for future reference
>>>>>>> 39e7b2489c8342788efbec413e1a18b46f81a409
...
=======
    model <- lm(formula = Secondary_CPUE ~ Primary_CPUE, data = CPUE_spec) #see comment above re: "Sample"
>>>>>>> 39e7b2489c8342788efbec413e1a18b46f81a409
...
=======
Results <- merge(Significant_Results, Small_Sample, by = c("Species", "Year"), all = TRUE) #so here is another object "Results"
#this can be dangerous

Results <- Results[!(is.na(Results$`p-value`)) | !(is.na(Results$Significance)) | !(is.na(Results$Warning)) | !(is.na(Results$`Sample Size`)), ]

Significant <- subset(Significant_Results, Significance == "Different Means")
>>>>>>> 39e7b2489c8342788efbec413e1a18b46f81a409
...
=======
conf_int <- cast(conf_int, Var1 + Var3 ~ Var2, value = "Freq") #also a good place to use a new name, the structure of the data is changing
>>>>>>> 39e7b2489c8342788efbec413e1a18b46f81a409
...
=======
means <- merge(mymeans, mymeans2, by = c("Species", "Year"), all = TRUE)#same comment here
>>>>>>> 39e7b2489c8342788efbec413e1a18b46f81a409
...
=======
pac_cod <- CPUE_data[CPUE_data$Common_Name == "Pacific Cod" & CPUE_data$Year == "2004",] #why is this 2004, when everything else is 2008? Hmm....I see, it doesn't line up as nicely. I'll think about this a bit more when I look at the document
>>>>>>> 39e7b2489c8342788efbec413e1a18b46f81a409
...
=======
    "Polycorder_Number", all.x = TRUE) #new object names might be good in this section, there are four "loops" and if you arent careful, that can get in trouble.....a bad habit of mine, that I'm trying to break. It's ok sometimes, but when you start merging or changing the structure of the data, it's a good practice to create a new object. This probably originated from my original code ;-P
>>>>>>> 39e7b2489c8342788efbec413e1a18b46f81a409
...
=======
station <- cbind(station, ineffhks) #here it's less of a problem, you aren't changing the data, but just adding a column (as long as you don't accidentally do it more than once, in which case you will end up with multiple "ineffhks" columns)
>>>>>>> 39e7b2489c8342788efbec413e1a18b46f81a409
...
conf_int_all <- merge(conf_int_all, means, by = "Species", all = TRUE)
...
for(i in unique(boot_data$Common_Name)){
  ...
  mean[i] <- type_boot$t0
=======
diff_mean_results <- NULL#"mean" is a function name, changed to avoid potential overwrite errors, same comment in IPHC section

for(i in unique(boot_data$Common_Name)){ #OK, well this for loop taught me something, I've never seen them set up this way. Cool the way it assigns the row names.
  boot_spec <- subset(boot_data, boot_data$Common_Name == i)
  #if (nrow(boot_spec) < 3) {next}
  type_boot <- boot(boot_spec, diff.means, R = 1000, strata = boot_spec[ ,4])
  if(sd(type_boot$t[, 1]) != 0 & sd(type_boot$t[, 2]) != 0){
  ...
  diff_mean_results[i] <- type_boot$t0 #make sure that this is pulling the correct info, in the IPHC section you had type_boot$t0[1], here you have type_boot$t0. Why?
>>>>>>> 39e7b2489c8342788efbec413e1a18b46f81a409
}
...
boot_result <- merge(mean, boot_result, by = "row.names", all = TRUE)

colnames(boot_result) <- c("Species", "Mean Difference", "Lower Bound", "Upper Bound")

boot_result$`Mean Difference` <- signif(boot_result$`Mean Difference`, 2)
=======
boot_result <- subset(boot_result, select = c("V4", "V5")) #suggest new object name here because removing data

diff_mean_results <- data.frame(diff_mean_results)

boot_result <- na.omit(boot_result)

boot_result <- merge(diff_mean_results, boot_result, by = "row.names", all = TRUE) #suggest new object name here because of merging two datasets

colnames(boot_result) <- c("Species", "Mean Difference", "Lower Bound", "Upper Bound") #this doesn't work because there are too few column names

boot_result$`Mean Difference` <- signif(boot_result$`Mean Difference`, 2) #It looks like this function isn't going to give the desired output format. I think that it is preferable to have consistent decimal places. Maybe multiply all the values by 100 and report in the caption that they are 10e-3. This comment will probably apply throughout, so I'll think about it a bit and comment again when I review the text. 
>>>>>>> 39e7b2489c8342788efbec413e1a18b46f81a409
