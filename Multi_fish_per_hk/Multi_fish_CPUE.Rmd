...
#
Y <- 11 #what values does this represent? Suggest not hard coding this value
...
#
for(y in 1:Y){ #I changed the 11 to Y so the code is more flexible to data changes, same for I
  CPUE_year <- subset(CPUE_data, CPUE_data$Year == unique(CPUE_data$Year)[[y]]) #I don't think the double square brackets are necessary?
  for(i in 1:I){ #this is set up a little weird, you have i through 66, but there aren't 66 species in the CPUE_year loop data
    #suggest having it 1:length(unique$CPUE_year)
    CPUE_spec <- subset(CPUE_year, CPUE_year$Common_Name == unique(CPUE_data$Common_Name)[[i]]) #this is going to look for things that aren't there because you are looking at the species list from the larger data set
    #suggest: CPUE_spec<-CPUE_year[CPUE_year$Common_Name==unique(CPUE_year)[i],]
    CPUE_spec <- na.omit(CPUE_spec)
    spec <- unique(CPUE_data$Common_Name)[[i]]
    year <- unique(CPUE_data$Year)[[y]]
