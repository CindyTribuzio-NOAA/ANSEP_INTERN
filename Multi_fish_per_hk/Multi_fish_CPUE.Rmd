...
# Add "*" for all significant results

conf_guide <- which(conf_int_all$`Primary Upper CPUE` < conf_int_all$`Secondary Lower CPUE` | conf_int_all$`Secondary Upper CPUE` < conf_int_all$`Primary Lower CPUE`)

conf_int_all$`Primary Mean CPUE` <- sprintf("%.4f", round(conf_int_all$`Primary Mean CPUE`, 4))

conf_int_all$`Primary Lower CPUE` <- sprintf("%.4f", round(conf_int_all$`Primary Lower CPUE`, 4))

conf_int_all$`Primary Upper CPUE` <- sprintf("%.4f", round(conf_int_all$`Primary Upper CPUE`, 4))

conf_int_all$`Secondary Mean CPUE` <- sprintf("%.4f", round(conf_int_all$`Secondary Mean CPUE`, 4))

conf_int_all$`Secondary Lower CPUE` <- sprintf("%.4f", round(conf_int_all$`Secondary Lower CPUE`, 4))

conf_int_all$`Secondary Upper CPUE` <- sprintf("%.4f", round(conf_int_all$`Secondary Upper CPUE`, 4))
...
conf_int_all <- conf_int_all[, c("Species", "Year", "Primary Mean CPUE (Lower Bound, Upper Bound)", "Secondary Mean CPUE (Lower Bound, Upper Bound)")]

conf_int_all$`Primary Mean CPUE (Lower Bound, Upper Bound)`[conf_guide] <- sapply(conf_int_all$`Primary Mean CPUE (Lower Bound, Upper Bound)`[conf_guide], function(x) paste(as.character(x), "*"))

conf_int_all$`Secondary Mean CPUE (Lower Bound, Upper Bound)`[conf_guide] <- sapply(conf_int_all$`Secondary Mean CPUE (Lower Bound, Upper Bound)`[conf_guide], function(x) paste(as.character(x), "*"))
...
boot_guide <- which(boot_result$`Upper Bound` < 0 | boot_result$`Lower Bound` > 0)

boot_result$`Mean Difference` <- sprintf("%.4f", round(boot_result$`Mean Difference`, 4))

boot_result$`Lower Bound` <- sprintf("%.4f", round(boot_result$`Lower Bound`, 4))

boot_result$`Upper Bound` <- sprintf("%.4f", round(boot_result$`Upper Bound`, 4))
...
boot_result <- boot_result[, c("Species", "Year", "Mean Difference (Lower Bound, Upper Bound)")]

boot_result$`Mean Difference (Lower Bound, Upper Bound)`[boot_guide] <- sapply(boot_result$`Mean Difference (Lower Bound, Upper Bound)`[boot_guide], function(x) paste(as.character(x), "*"))
#===============================================================
#===============================================================
```
...
conf_guide2 <- which(conf_int_all2$`Primary Upper CPUE` < conf_int_all2$`Secondary Lower CPUE` | conf_int_all2$`Secondary Upper CPUE` < conf_int_all2$`Primary Lower CPUE`)

conf_int_all2$`Primary Mean CPUE` <- sprintf("%.4f", round(conf_int_all2$`Primary Mean CPUE`, 4))

conf_int_all2$`Primary Lower CPUE` <- sprintf("%.4f", round(conf_int_all2$`Primary Lower CPUE`, 4))

conf_int_all2$`Primary Upper CPUE` <- sprintf("%.4f", round(conf_int_all2$`Primary Upper CPUE`, 4))

conf_int_all2$`Secondary Mean CPUE` <- sprintf("%.4f", round(conf_int_all2$`Secondary Mean CPUE`, 4))

conf_int_all2$`Secondary Lower CPUE` <- sprintf("%.4f", round(conf_int_all2$`Secondary Lower CPUE`, 4))

conf_int_all2$`Secondary Upper CPUE` <- sprintf("%.4f", round(conf_int_all2$`Secondary Upper CPUE`, 4))
...
conf_int_all2$`Primary Mean CPUE (Lower Bound, Upper Bound)`[conf_guide2] <- sapply(conf_int_all2$`Primary Mean CPUE (Lower Bound, Upper Bound)`[conf_guide2], function(x) paste(as.character(x), "*"))

conf_int_all2$`Secondary Mean CPUE (Lower Bound, Upper Bound)`[conf_guide2] <- sapply(conf_int_all2$`Secondary Mean CPUE (Lower Bound, Upper Bound)`[conf_guide2], function(x) paste(as.character(x), "*"))
...
boot_guide2 <- which(boot_result2$`Upper Bound` < 0 | boot_result2$`Lower Bound` > 0)

boot_result2$`Mean Difference` <- sprintf("%.4f", round(boot_result2$`Mean Difference`, 4))

boot_result2$`Lower Bound` <- sprintf("%.4f", round(boot_result2$`Lower Bound`, 4))

boot_result2$`Upper Bound` <- sprintf("%.4f", round(boot_result2$`Upper Bound`, 4))
...
boot_result2$`Mean Difference (Lower Bound, Upper Bound)`[boot_guide2] <- sapply(boot_result2$`Mean Difference (Lower Bound, Upper Bound)`[boot_guide2], function(x) paste(as.character(x), "*"))
#===============================================================
#===============================================================
```
...
#===============================================================
#===============================================================
# Mark every significant result with "*"

guide <- which(Results$`p-value` < 0.05)

Results$`p-value` <- sprintf("%.4f", round(Results$`p-value`, 4))

rounding <- which(Results$`p-value` == "0.0000")

Results$`p-value`[rounding] <- sapply(Results$`p-value`[rounding], function(x) paste("<0.0001"))

Results$`p-value`[guide] <- sapply(Results$`p-value`[guide], function(x) paste(as.character(x), "*"))

# Remove Warning and Significance column since they are only for analysis

Results <- Results[, c("Species", "Year", "p-value", "Sample Size")]

# Create table of results

kable(Results, caption = "Table 1 --  IPHC. All of the species analyzed in the study by year, presenting the p-value for the test performed and the sample size for that species where sample size is the number of stations with that species/year combination.", digits = 4, row.names = FALSE, format.args = list(na.encode = TRUE))
```
...
######*Intervals do not overlap prior to rounding
...
######*Interval does not contain 0 prior to rounding
...
spec_data <- CPUE_subset[CPUE_subset$Common_Name %in% c("Thornyhead", "Octopus", "Darkblotched Rkfish", "Sixgill Shark"),]

spec_data$Deviation <- spec_data$Secondary_CPUE - spec_data$Primary_CPUE

different <- which(spec_data$Primary_CPUE != spec_data$Secondary_CPUE)

plot_data <- spec_data[different, ]

spec_data <- spec_data[order(spec_data$Common_Name),]

spec_data$Species <- factor(spec_data$Common_Name, levels = c("Octopus", "Thornyhead", "Sixgill Shark", "Darkblotched Rkfish"))

plot_data <- plot_data[order(plot_data$Common_Name),]

plot_data$`Deviation` <- with(plot_data, as.vector(unlist(tapply(Deviation, Common_Name, function(x) (x - min(x))/(max(x) - min(x))))))

plot_data$`Deviation`[is.nan(plot_data$`Deviation`)] <- 0.5

plot_data$Species <- factor(plot_data$Common_Name, levels = c("Octopus", "Thornyhead", "Sixgill Shark", "Darkblotched Rkfish"))

spec_data$Set <- "Anomaly"

plot_data$Set <- "Standardized Anomaly"

set_data <- rbind(spec_data, plot_data)

plot_image <- ggplot(set_data, aes(Species, Deviation, col = factor(Year), shape = Species)) + geom_point(position = "jitter") + labs(x = "Species", y = "Anomaly", col = "Year") + theme_bw() + facet_wrap( ~ Set, scales = "free_y", nrow = 2)
...
guide <- which(Results2$`p-value` < 0.05)

Results2$`p-value` <- sprintf("%.4f", round(Results2$`p-value`, 4))

rounding <- which(Results2$`p-value` == "0.0000")

Results2$`p-value`[rounding] <- sapply(Results2$`p-value`[rounding], function(x) paste("<0.0001"))

Results2$`p-value`[guide] <- sapply(Results2$`p-value`[guide], function(x) paste(as.character(x), "*"))

# Remove 'Significance' column as it is only used for analysis

Results2 <- Results2[, c("Species", "p-value", "Sample Size")]

# Create a table of the results

kable(Results2, caption = "Table 4 --  NMFS. All of the species analyzed in the study, presenting the p-value for the test performed and the sample size for that species where the sample size is the number of stations across which the mean CPUE was calculated.", digits = 3, row.names = FALSE, format.args = list(na.encode = TRUE))
```
...
spec_data2 <- catch_data6[catch_data6$Common_Name %in% c("Shortspine thornyhead", "Octopus", "Lingcod", "Pacific cod"),]

spec_data2$Deviation <- spec_data2$secCPUE - spec_data2$priCPUE

different2 <- which(spec_data2$priCPUE != spec_data2$secCPUE)

plot_data2 <- spec_data2[different2, ]

spec_data2 <- spec_data2[order(spec_data2$Common_Name),]

spec_data2$Species <- factor(spec_data2$Common_Name, levels = c("Octopus", "Shortspine thornyhead", "Lingcod", "Pacific cod"))

plot_data2 <- plot_data2[order(plot_data2$Common_Name),]

plot_data2$`Deviation` <- with(plot_data2, as.vector(unlist(tapply(Deviation, Common_Name, function(x) (x - min(x))/(max(x) - min(x))))))

plot_data2$`Deviation`[is.nan(plot_data2$`Deviation`)] <- 0.5

plot_data2$Species <- factor(plot_data2$Common_Name, levels = c("Octopus", "Shortspine thornyhead", "Lingcod", "Pacific cod"))

spec_data2$Set <- "Anomaly"

plot_data2$Set <- "Standardized Anomaly"

set_data2 <- rbind(spec_data2, plot_data2)

plot_image2 <- ggplot(set_data2, aes(Species, Deviation, shape = Species)) + geom_point(position = "jitter") + labs(x = "Species", y = "Anomaly") + theme_bw() + facet_wrap( ~ Set, scales = "free_y", nrow = 2)
